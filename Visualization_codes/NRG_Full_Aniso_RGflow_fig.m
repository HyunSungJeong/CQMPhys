function NRG_Full_Aniso_RGflow_fig(J0, K_perp, K_z, I_perp, I_z, FitInfo, varargin)
    ...
    Nkeep = 3000;
    Lambda = 2.5;
    TsoK_path = 'C:\Users\hsjun\OneDrive\Physics\Research\data\Archive';
    Data_path = ['J0=', sprintf('%.15g',J0), '_K_perp=', sprintf('%.15g',K_perp), '_K_z=', sprintf('%.15g',K_z), ...
                   '_I_perp=', sprintf('%.15g', I_perp),'_I_z=', sprintf('%.15g', I_z),'_T=1e-20_Nkeep=3000'];
    ...
    tmp = load([Full_path, filesep, 'NRG_Op=ImpSpOrb_plus.mat']);    ImpSusc{4} = tmp.temp;
    tmp = load([Full_path, filesep, 'NRG_Op=ImpSpOrb_z.mat']);    ImpSusc{5} = tmp.temp;
    ...
    tmp = load([Full_path, filesep, 'NRG_Op=BathSpOrb_plus.mat']);    BathSusc{4} = tmp.temp;
    tmp = load([Full_path, filesep, 'NRG_Op=BathSpOrb_z.mat']);    BathSusc{5} = tmp.temp;
