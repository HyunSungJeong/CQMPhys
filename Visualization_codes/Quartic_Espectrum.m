function Espec = Quartic_Espectrum(Lambda_z, Lambda_x, N_iter, Nshow)
    % <Description>
    ...
    %
    % <Output>
    % Espec : [table] Table containing the low-energy spectrum of the 4-electron interaction Kondo model.
    %               The energy levels are normalized so that the ground state energy is 0 and the first excited state energy is 1

    path = [fileparts(mfilename('fullpath')),filesep,'QuarticData_Espectrum',filesep,'lambda_z=', num2str(Lambda_z), '_lambda=', num2str(Lambda_x), '_T=1e-24_Nkeep=3000'];
    ...
    SU2spin_1 = round(Elev(:,4));
    SU2spin_2 = round(Elev(:,5));
