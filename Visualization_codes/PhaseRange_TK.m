function [K, Phase_range, Phase_name, varargout] = PhaseRange_TK(J0,I0,varargin)
    % <Description>
    % Returns the range and corresponding temperature range for data of given (J0, J0)
    %
    % <Input>
    % J0 : [numeric] orbital-orbital coupling for the isotropic 2-channel spin-orbital Kondo(2soK) model
    ...
    RelevData = DataFolders(RelevIdx);      % folder names of data that are relevant for plotting: ones that are (J, K) = (J0, J0)
    K = K(RelevIdx);                        % parameters 'J' that are available for plotting
    T = T(RelevIdx);
    ...
    for itSub = 1:numel(RelevData)        % for all data with relevant parameters
        ...
        for itD = 1:numel(FileInfo)     % load impurity susceptibilities and entropy for each relevant parameter
                ...
                switch FileInfo(itD).name
                    ...
                    case 'ContData_alphaz=1.mat'
                        tmp = load([DataPath,'\ContData_alphaz=1.mat']);
                        field = fieldnames(tmp);
        ...
        if HaveDisc
            ...
            if HaveCont
                ...
                for ita = (1:size(Adiscs,1))
                    ...
                    [ocont{itSub}, Aconts{ita}] = getAcont(odisc,Adisc,sigmak,T(itSub)/5,'alphaz',1,'emin',emin);
                end
                ...
                save([path, filesep, RelevData{itSub}, filesep, 'ContData_alphaz=1.mat'], 'tmp');
            end
        end
    ...
    if plotSusc
        ...
        for itP = 1:Nplot
            ...
            for itSub = 1:Nsub
                ...
                title(['$\left(J_{0},K_{0},I_{0}\right) = \left(',sprintf('%.15g',J0),', ',sprintf('%.15g',K(DataNum)),', ',sprintf('%.15g',I0),' \right)$'], ...
