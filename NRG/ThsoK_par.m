function ThsoK_par (varargin)
    ...
    for it = (1:num_jobs)
      if ~exist(['/data/',getenv('USER'),'/ThsoK/',partot(it).JobName,'_Nkeep=',sprintf('%.15g',Nkeep)],'dir')
        mkdir(['/data/',getenv('USER'),'/ThsoK/',partot(it).JobName,'_Nkeep=',sprintf('%.15g',Nkeep)]);
      end
    end
    ...
    parfn = ['ThsoK_par_J0=',J0s,'_K0=',K0s,'_I0=',I0s,'_T=',Ts];
