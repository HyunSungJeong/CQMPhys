function [I_avail, Phase_range, Phase_name, varargout] = PhaseRange_2soK_Aniso(J0, K_perp, K_z, varargin)
    % <Description>
    % Returns the range and corresponding temperature range for data of given (J0, K_perp, K_z)
    %
    % <Input>
    % J0 : [numeric] spin-spin coupling for the anisotropic 2-channel spin-orbital Kondo(2soK) model 
    %           in which the phase range will be evaluated
    %
    % K_perp : [numeric] orbital-orbital coupling in the perpendicular direction for the anisotropic 
    %               2-channel spin-orbital Kondo(2soK) model in which the phase range will be evaluated
    %
    % K_z : [numeric] orbital-orbital coupling in the z-direction for the anisotropic 
    %               2-channel spin-orbital Kondo(2soK) model in which the phase range will be evaluated
    %
    % <Options>
    ...
    J_avail = zeros(1,numel(DataFolders));
    K_perp_avail = J_avail;
    K_z_avail = J_avail;
    I_avail = J_avail;
    T_avail = J_avail;
    for it = 1:numel(DataFolders)
        ...
        J_avail(it) = tmp(1);
        K_perp_avail(it) = tmp(2);
        K_z_avail(it) = tmp(3);
        I_avail(it) = tmp(4);
        T_avail(it) = tmp(5);
    end
    [I_avail, idx] = sort(I_avail,'ascend');
    J_avail = J_avail(idx);
    K_perp_avail = K_perp_avail(idx);
    K_z_avail = K_z_avail(idx);
    T_avail = T_avail(idx);
    DataFolders = DataFolders(idx);

    %% Load data to be used for plotting
    if isnan(I0_fixed)
        RelevIdx = (J_avail == J0) & (K_perp_avail == K_perp) & (K_z_avail == K_z) & (abs(I_avail) < 0.2) & (abs(I_avail) >= I0min);       % Indices of folders that are relevant for plotting
    else
        RelevIdx = (J_avail == J0) & (K_perp_avail == K_perp) & (K_z_avail == K_z) & (I_avail == I0_fixed);
    end
    RelevData = DataFolders(RelevIdx);      % folder names of data that are relevant for plotting
    I_avail = I_avail(RelevIdx);            % parameters 'I' that are available for plotting
    T_avail = T_avail(RelevIdx);
    ...
    for itSub = 1:numel(RelevData)        % for all data with relevant parameters
        ...
        if HaveDisc
            ...
            if HaveCont
                ...
                ImpOrb_plus{itSub} = ContData.ImpOrb_plus;
                ImpOrb_z{itSub} = ContData.ImpOrb_z;
            else
                ...
                for ita = (1:size(Adiscs,1))
                    ...
                    [ocont{itSub}, Aconts{ita}] = getAcont(odisc,Adisc,sigmak,T_avail(itSub)/5,'alphaz',alphaz,'emin',emin);
                end
    
                ImpSp{itSub} = Aconts{1};
                ImpOrb_plus{itSub} = Aconts{2};
                ImpOrb_z{itSub} = Aconts{3};
    
                tmp = [];
                tmp.ImpSp = Aconts{1};
                tmp.ImpOrb_plus = Aconts{2};
                tmp.ImpOrb_z = Aconts{2};
        ...
        if DataNum >= 6
            ...
            ImpOrb_plus{DataCnt} = ImpOrb_plus{itSub};
            ImpOrb_z{DataCnt} = ImpOrb_z{itSub};
            Sent{DataCnt} = Sent{itSub};

            J_avail(DataCnt) = J_avail(itSub);
            K_perp_avail(DataCnt) = K_perp_avail(itSub);
            K_z_avail(DataCnt) = K_z_avail(itSub);
            I_avail(DataCnt) = I_avail(itSub);
            T_avail(DataCnt) = T_avail(itSub);
        end
    ...
    ImpOrb_plus = ImpOrb_plus(1:DataCnt);
    ImpOrb_z = ImpOrb_z(1:DataCnt);
    Sent = Sent(1:DataCnt);

    J_avail = J_avail(1:DataCnt);
    K_perp_avail = K_perp_avail(1:DataCnt);
    K_z_avail = K_z_avail(1:DataCnt);
    I_avail = I_avail(1:DataCnt);
    T_avail = T_avail(1:DataCnt);
    ...
    for itSub = 1:N_Relev
        ...
        ImpDynSusc = {ImpSp{itSub},ImpOrb_plus{itSub}};  % impurity dynamic susceptibilities (spin, orbital)
        ...
        Phase_range{itSub} = [];
        Phase_name{itSub} = {};
        logT = log10(ocont{itSub}(ocont{itSub}>0));
        logSp = log10(ImpSp{itSub}(ocont{itSub}>0));
        logOrb_plus = log10(ImpOrb_plus{itSub}(ocont{itSub}>0));
        logOrb_z = log10(ImpOrb_z{itSub}(ocont{itSub}>0));

        for it1 = 1:size(Range_Plateau{1},1)
            range_Sp = Range_Plateau{1}(it1,:);

            for it2 = 1:size(Range_Plateau{2},1)
                range_Orb_plus = Range_Plateau{2}(it2,:);

                maxMin = max(range_Sp(1), range_Orb_plus(1));
                minMax = min(range_Sp(2), range_Orb_plus(2));
                if maxMin < minMax      % if plateau range is well-defined    

                    [ExpSp, ~] = logFit(logT,logSp,[maxMin,minMax]);
                    [ExpOrb_plus, ~] = logFit(logT,logOrb_plus,[maxMin,minMax]);
                    [ExpOrb_z, ~] = logFit(logT,logOrb_z,[maxMin,minMax]);

                    eps = 0.1;  % a small number
                    if abs(ExpSp-1) < eps && abs(ExpOrb_plus-1) < eps
                        Phase_range{itSub} = cat(1,Phase_range{itSub},[-24, minMax]);
                        Phase_name{itSub} = cat(1,Phase_name{itSub},{'Fermi Liquid'});

                    elseif abs(ExpSp) < eps && abs(ExpOrb_plus) < eps
                        Phase_range{itSub} = cat(1,Phase_range{itSub},[maxMin, minMax]);
                        Phase_name{itSub} = cat(1,Phase_name{itSub},{'Fully Overscreened'});
                        
                    elseif ExpSp < -1 && ExpOrb_plus < -1
        ...
        for it = 1:size(Range_Plateau{1},1)
            ...
            [ExpOrb_plus, ~] = logFit(logT,logOrb_plus,Range_Plateau{1}(it,:));
            
            if abs(ExpSp) < eps && abs(ExpOrb_plus) > eps
                if abs(ExpOrb_z + 1) < eps
                    Phase_range{itSub} = cat(1, Phase_range{itSub}, Range_Plateau{1}(it,:));
                    Phase_name{itSub} = cat(1, Phase_name{itSub}, {'Spin Overscreened'});
                else
                    Phase_range{itSub} = cat(1, Phase_range{itSub}, Range_Plateau{1}(it,:));
                    Phase_name{itSub} = cat(1, Phase_name{itSub}, {'Orbital-Ferromagnetic Spin-Overscreened'});
                end
            end
        end
        ...
        for it = 1:size(Range_Plateau{2},1)
            ...
            [ExpOrb_plus, ~] = logFit(logT,logOrb_plus,Range_Plateau{2}(it,:));

            if abs(ExpOrb_plus) < eps && abs(ExpSp) > eps
        ...
        if InflecBound
            ...
            T_grid = ocont{itSub}(ocont{itSub} > 0);
            Orb_plus_Susc = ImpOrb_plus{itSub}(ocont{itSub} > 0);
            [logT, Orb_2Der] = log_Susc_2ndDer(T_grid, Orb_plus_Susc);
            ...
            elseif ~isempty(SpInflec)
                ...
                elseif ismember('Orbital-Ferromagnetic Spin-Overscreened', Phase_name{itSub})

                    Idx = find(strcmp(Phase_name{itSub}, 'Orbital-Ferromagnetic Spin-Overscreened'));
                    Phase_range{itSub}(Idx,2) = SpInflec;
                end
            end
    ...
    if plotSusc
        ...
        for itP = 1:Nplot
            ...
            for itSub = 1:Nsub
                ...
                hold on;
                PosIdx = ImpSp{DataNum} > 0 & ImpOrb_plus{DataNum};
                if ~isempty(ocont{DataNum})
                    Sp = plot(ocont{DataNum}(PosIdx),ImpSp{DataNum}(PosIdx),'color','blue','Linewidth',1,'LineStyle','-');
                    Orb_plus = plot(ocont{DataNum}(PosIdx),ImpOrb_plus{DataNum}(PosIdx),'color','red','Linewidth',1,'LineStyle','--');
                end
                set(ax,'XScale','log','YScale','log');
                title(['$\left(J_{0},K_{\perp},K_{z},I_{0}\right) = \left(',sprintf('%.15g',J0),', ',sprintf('%.15g',K_perp),', ',sprintf('%.15g',K_z),', ',SciNot(I_avail(DataNum),'Signif',1),' \right)$'], ...
            ...
            handles = [Sp, Orb_plus, Phase_handles{ExistPhase}];
            ...
            for itSub = 1:Nsub
                ...
                [~, Orb_2ndDer] = log_Susc_2ndDer(ocont{DataNum}(PosIdx), ImpOrb_plus{DataNum}(PosIdx));    % second derivative of impurity spin dynamic susceptibility
                ...
                if ~isempty(ocont{DataNum})
                    ...
                    Orb_plus = plot(power(10,logT),Orb_2ndDer_mov,'color','red','Linewidth',1,'LineStyle','--');
                end
