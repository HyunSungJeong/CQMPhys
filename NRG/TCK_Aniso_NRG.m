function TCK_Aniso_NRG(parfn,varargin)
    ...
    [FF,ZF,J,IF] = getLocalSpace('FermionS','Acharge(:),Aspin','NC',2);
    [Fs,Zs,J,Es] = setItag('s00','op',FF(:),ZF,J(:),IF.E);
    J_sp_plus = -sqrt(2)*J(1);
    J_sp_minus = sqrt(2)*J(2);
    J_sp_z = J(3);
    ...
    S_plus = getsub(J_sp_plus, find(all((J_sp_plus.Q{1}(:,1) ==0 & J_sp_plus.Q{1}(:,2) == -1), 2)));
    S_minus = getsub(J_sp_minus, find(all((J_sp_minus.Q{1}(:,1) ==0 & J_sp_minus.Q{1}(:,2) == -1), 2)));
    S_z = getsub(J_sp_z, find(all((J_sp_z.Q{1}(:,1) ==0 & J_sp_z.Q{1}(:,2) == -1), 2)));

    N2 = contract(Fs(2),'!2*',Fs(2)) + contract(Fs(4),'!2*',Fs(4));         % Number op. for site 2
    ...
    STG = ['/data/',getenv('USER'),'/TCK_Aniso/',JobName,'_Nkeep=',sprintf('%.15g',Nkeep)];
